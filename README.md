...
从 [Releases](../../releases) 下载 `SteamGameActivator.exe`，然后运行：

```powershell
.\SteamGameActivator.exe
```
...
```powershell
.\SteamGameActivator.exe install
.\SteamGameActivator.exe activate APPID
```
...
输出文件位于 `dist/SteamGameActivator.exe`。三个 DLL 会作为资源嵌入最终 EXE。
